...
az network vpn-gateway connection create --gateway-name demo-we-hub-vpngw --name onprem2 --remote-vpn-site onprem2 -g vwan-ri-terraform-rg --shared-key $sharedkey --enable-bgp true --no-wait
...
az network local-gateway create -g vwan-ri-terraform-rg -n lng2 --gateway-ip-address $hubgwtunneladdress --location westeurope --asn $hubgwasn --bgp-peering-address $hubgwbgpaddress

echo "# VNET GW: connect from vnet gw to local network gateway"
az network vpn-connection create -n to-we-hub --vnet-gateway1 qonprem2-gw -g vwan-ri-terraform-rg --local-gateway2 lng2 -l westeurope --shared-key $sharedkey --enable-bgp
