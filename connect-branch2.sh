...
vnetgwtunnelip1=$(az network vnet-gateway show -n qonprem2-gw -g vwan-terraform-rg --query "bgpSettings.bgpPeeringAddresses[0].tunnelIpAddresses[0]" --output tsv)
echo "VNET GW Tunnel address #1:" $vnetgwtunnelip1
vnetgwbgpip1=$(az network vnet-gateway show -n qonprem2-gw -g vwan-terraform-rg --query "bgpSettings.bgpPeeringAddresses[0].defaultBgpIpAddresses"  --output tsv)
echo "VNET GW BGP address:" $vnetgwbgpip1
vnetgwasn=$(az network vnet-gateway show -n qonprem2-gw -g vwan-terraform-rg --query "bgpSettings.asn" --output tsv)
...
hubgwbgpaddress=$(az network vpn-gateway show --name demo-eastus-hub-vpngw  -g vwan-terraform-rg --query "bgpSettings.bgpPeeringAddresses[?ipconfigurationId == 'Instance0'].defaultBgpIpAddresses" --output tsv)
echo "Hub GW BGP address:" $hubgwbgpaddress
hubgwasn=$(az network vpn-gateway show --name demo-eastus-hub-vpngw  -g vwan-terraform-rg --query "bgpSettings.asn" --output tsv)
echo "Hub GW BGP ASN:" $hubgwasn
hubgwkey=$(az network vpn-gateway connection show --gateway-name demo-eastus-hub-vpngw --name onprem2 -g vwan-terraform-rg --query "sharedKey" --output tsv)
